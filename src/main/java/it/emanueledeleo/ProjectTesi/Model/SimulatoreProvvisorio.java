package it.emanueledeleo.ProjectTesi.Model;
...
import it.emanueledeleo.ProjectTesi.Model.Mezzo.Stato;
...
public class SimulatoreProvvisorio {
	...
	PriorityQueue<Ordine> ordiniInAttesa;
	...
	public void init(DijkstraShortestPath<Citta, Arco> dijkstra, Graph<Citta, Arco> grafo,
		...
		this.ordiniInAttesa = new PriorityQueue<Ordine>();
	...
	public void runMezzo() {
		...
		if (diff >= 5) {
			...
			LocalDateTime localData = LocalDateTime.of(startDate.getYear(), startDate.getMonth(), startDate.getDay(),
			...
			for (int id : mapMezzi.keySet()) {
				...
				if (localData.isAfter(mapMezzi.get(id).getDataMezzo())
						&& mapMezzi.get(id).getStato().equals(Stato.OCCUPATO)) {
				...
				if (mapMezzi.get(id).getStato().equals(Stato.DISPONIBILE)
					...
					while (!mapMezzi.get(id).getOrdiniMezzo().isEmpty()) {
						...
						long secondi = Math.round((ordineDaGestire.getProssimaTratta().getDistanza()
								/ mapMezzi.get(id).getVelocitaMedia()) * 3600);
	...
	private void processEvent(Event e) {
		...
		case NUOVO_ORDINE:
			...
			if (currentOnDB.isProcessable()) {
				...
				this.queue.add(
						new Event(nuovoOrdine, EventType.ORDINE_IN_CORSO, nuovoOrdine.getDataOra().plusHours(timeout)));
